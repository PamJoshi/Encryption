...
async def download_file(temp_id: str, filename: str):
    ...
    # Add endpoint to download encrypted/decrypted files
    @app.get("/download/{temp_id}/{filename}")
    async def download_file(temp_id: str, filename: str):
        # Decode filename in case it is URL-encoded
        safe_filename = urllib.parse.unquote(filename)
        temp_dir = os.path.join(tempfile.gettempdir(), temp_id)
        # Try both root and subfolders (encrypted/decrypted)
        possible_paths = [
            os.path.join(temp_dir, safe_filename),
            os.path.join(temp_dir, "encrypted", safe_filename),
            os.path.join(temp_dir, "decrypted", safe_filename),
        ]
...
async def download_file(temp_id: str, filename: str):
    ...
    # If not found, try 'decrypted' subfolder
    if not os.path.exists(file_path):
        file_path = os.path.join(temp_dir, 'decrypted', filename)
    # If not found, try 'encrypted' subfolder (for encrypted downloads)
    if not os.path.exists(file_path):
        file_path = os.path.join(temp_dir, 'encrypted', filename)
    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail="File not found")
